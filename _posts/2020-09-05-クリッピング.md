...
categories: 機能
layout: post
permalink: /articles/クリッピング/
---
管理人が個人的に利用する為に実装した機能ですが、もしご興味があれば、ご利用ください。

## 機能概要

- 公開[キュア！]({{ "/articles/キュア！/" | relative_url }})を、MarkdownファイルとしてDropboxに保存（クリップ）します。
...
![]({{ "/images/クリッピング/トークン.png" | relative_url }})
...
- 登録済みの場合は、[環境設定](https://precure.ml/mulukhiya/app/config)画面へ。
![]({{ "/images/クリッピング/HOME.png" | relative_url }})
...
![]({{ "/images/クリッピング/ブックマーク操作1.png" | relative_url }})

![]({{ "/images/クリッピング/ブックマーク操作2.png" | relative_url }})
...
![]({{ "/images/クリッピング/Ulysses.png" | relative_url }})


## 利用上の注意

- ~~クリップできるのは、「公開」の[キュア！]({{ "/articles/キュア！/" | relative_url }})だけです。「未収載」以下の場合は、Mastodon本来のブックマークだけが保存されます。~~ Dropboxは個人用途であり、クリッピングが即公開につながるわけでもないので、未収載以下のキュア！もMarkdownで保存できるようにしました。
